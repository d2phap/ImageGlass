...
public partial class GDIExtractor : IExtractor
{
    ...
    private static Image ScaleDownRotateBitmap(Image source, double scale, int angle)
    {
        ...
        // Scale
        double xScale = Math.Min(1.0, Math.Max(1.0 / sourceWidth, scale));
        double yScale = Math.Min(1.0, Math.Max(1.0 / sourceHeight, scale));

        int width = (int)(sourceWidth * xScale);
        int height = (int)(sourceHeight * yScale);
